...
public sealed class HexClientStoreTests
{
	...
	public void CompleteStatePublicationAtomicallyTransitionsAndUnloadsCharacter()
	{
		...
		store.BeginSession();
		store.ReplaceCharacterList( characterList );
		Assert.IsTrue( store.ApplyState( State( connection, 1, 1, characterId, new[] { Inventory() }, Action() ) ) );
		store.ReplaceChat( chat );
		...
		Assert.IsTrue( store.ApplyState( State( connection, 2, 2, null ) ) );
	...
	public void CharacterSwitchReplacesEveryCharacterScopedViewInOnePublication()
	{
		...
		store.ApplyState( State( connection, 1, 1, first, new[] { Inventory( "First" ) }, Action() ) );
		store.ReplaceChat( Chat( new ChatDeliveryEpoch( connection, 1 ) ) );

		var secondInventory = Inventory( "Second" );
		Assert.IsTrue( store.ApplyState( State( connection, 2, 2, second, new[] { secondInventory } ) ) );
	...
	public void LateRevisionWrongConnectionAndUnversionedCharacterSwitchAreRejected()
	{
		...
		Assert.IsTrue( store.ApplyState( accepted ) );

		Assert.IsFalse( store.ApplyState( State( firstConnection, 4, 9, character ) ) );
		Assert.IsFalse( store.ApplyState( State( ConnectionEpoch.New(), 5, 11, CharacterId.New() ) ) );
		Assert.IsFalse( store.ApplyState( State( firstConnection, 4, 11, CharacterId.New() ) ) );
	...
	public void RosterAndSchemaViewsAreImmutableAtomicAndProtectedByTheSameEpoch()
	{
		...
		Assert.IsFalse( store.ApplyState( State(
			connection, 1, 1, character,
			roster: lateRoster,
	...
	public void BeginSessionIsInstanceScopedAndIndependentOfListenHostState()
	{
		...
		hostProcessStore.BeginSession();
		hostProcessStore.ApplyState( State(
			ConnectionEpoch.New(), 1, 1, hostCharacter, new[] { Inventory( "Host inventory" ) } ) );

		clientScopeStore.BeginSession();
		...
		Assert.AreEqual( ClientLifecycleState.Connected, clientScopeStore.Lifecycle );
	...
	public void OutOfOrderUniqueChatDeliveriesMergeWithoutLoweringRevisionOrDuplicatingMessages()
	{
		...
		Assert.IsTrue( store.ApplyState( State( connection, 1, 1, CharacterId.New() ) ) );
		...
		store.ReplaceChat( new ChatSnapshot( epoch, 4, new[] { first } ) );
		store.ReplaceChat( new ChatSnapshot( epoch, 6, new[] { third } ) );
		store.ReplaceChat( new ChatSnapshot( epoch, 5, new[] { second } ) );
		store.ReplaceChat( new ChatSnapshot( epoch, 3, new[] { first } ) );
	...
	public void SameRevisionChunksMergeAndRetentionKeepsNewestBoundedHistory()
	{
		...
		store.ReplaceChat( new ChatSnapshot( epoch, 10, messages.Take( 150 ) ) );
		store.ReplaceChat( new ChatSnapshot( epoch, 10, messages.Skip( 150 ) ) );
	...
	public void ReceiverStoresRemainIsolatedWhenHostPublishesDifferentRecipientSnapshots()
	{
		...
		Assert.IsTrue( firstReceiver.ApplyState( State( firstConnection, 1, 1, CharacterId.New() ) ) );
		Assert.IsTrue( secondReceiver.ApplyState( State( secondConnection, 1, 1, CharacterId.New() ) ) );
		firstReceiver.ReplaceChat( new ChatSnapshot( firstEpoch, 1, new[] { Message( "first-only", DateTimeOffset.UnixEpoch ) } ) );
		secondReceiver.ReplaceChat( new ChatSnapshot( secondEpoch, 1, new[] { Message( "second-only", DateTimeOffset.UnixEpoch ) } ) );
		firstReceiver.ReplaceChat( new ChatSnapshot( firstEpoch, 2, new[] { Message( "first-next", DateTimeOffset.UnixEpoch.AddSeconds( 1 ) ) } ) );
	...
	public void PriorCharacterAndConnectionEpochDeliveriesCannotEnterTheCurrentView()
	{
		...
		Assert.IsTrue( store.ApplyState( State( connection, 1, 1, firstCharacter ) ) );
		store.ReplaceChat( new ChatSnapshot(
			new ChatDeliveryEpoch( connection, 1 ), 8,
			new[] { Message( "first-character", DateTimeOffset.UnixEpoch ) } ) );
		Assert.IsTrue( store.ApplyState( State( connection, 2, 2, secondCharacter ) ) );
		var version = store.Version;
