...
- The Hexagon library is platform-whitelisted and owns the persistence protocol abstraction. The standalone-only HL2RP game supplies the raw OS `FileStream` lease, write-through, and atomic-rename adapter; this boundary now passes exact-current generated compilation plus isolated persistence commit/drain/recovery smoke and still awaits the final visible-editor pass.
