...
- The current-worktree cohort contains 19 findings and eight shared root causes.
  ...
  evidence is recorded below with later-source limitations called out.
...
test additions mean that snapshot is historical, not final-current-worktree
evidence. Draft PRs, hosted CI, branch protection, the final full-verifier rerun,
and the two-authenticated-client release gate remain pending.
...
| R-03 | Medium / reliability | HL2RP `Runtime/HL2RPMaintenanceSupervisor.cs` owns a single in-flight tick, coalesced pending signal, observed exceptions, `finally` cleanup, cancellation-aware disposal, and 250 ms-to-30 s exponential retry with immutable status. `HL2RPHostApplication.cs` initiates verification shutdown without awaiting its own supervised tick, avoiding self-shutdown deadlock. | Runtime composition tests exercise throw/retry, coalescing, backoff reset, disposal, and observed status; the recorded engine smoke reached clean shutdown after the self-shutdown regression was corrected. | Closed in implementation and locally verified; final current-source smoke rerun pending. |
...
| T-02 | Low / CI, configuration, and deployment | Hexagon `.github/workflows/neutral.yml` and HL2RP `.github/workflows/integration.yml` use .NET 10, locked restore, audit, Release tests, portable validation, warnings-as-errors, clean-diff checks, and exact lowercase 40-character `hexagon.lock.json` pinning. Full local verification and source-bound evidence publication remain separate. | Lock files and portable/full scripts are present; intended protected contexts are `hexagon / neutral`, `hl2rp / integration`, and `sbox / release-evidence`. The current lock remains on baseline `4dda1f72ab3267e1ee85c537f5f6f11c1d51f234` until the Hexagon remediation head is committed and pushed. | Local workflow implementation is present. Final lock advancement, draft PRs, hosted runs, branch-protection enforcement, and release-evidence status publication remain pending, so enforcement is not operationally closed. |
...
- That engine smoke passed transactional commit -> `HEXAGON_DRAINED` -> exact-sequence/digest recovery after correcting JSON escape-insensitive canonical payload comparison and a maintenance self-shutdown deadlock. It did not reuse the stale temp launcher entry from the original audit. Later source changes require a final full-verifier/editor rerun before this can be called exact-current-worktree evidence.
...
- Draft PRs have not been opened. Hosted workflow results, strict branch-protection state, and release-evidence status publication therefore remain pending and are not asserted by this register.
...
- The Hexagon library is platform-whitelisted and owns the persistence protocol abstraction. The standalone-only HL2RP game supplies the raw OS `FileStream` lease, write-through, and atomic-rename adapter; this boundary passes exact-current generated compilation and the earlier recorded engine smoke, and still awaits the final current-source persistence/editor smoke.
