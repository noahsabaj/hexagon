...
Draft PRs, hosted CI, branch protection, and the two-authenticated-client release
gate remain pending at this pre-publication snapshot.
...
| T-02 | Low / CI, configuration, and deployment | Hexagon `.github/workflows/neutral.yml` and HL2RP `.github/workflows/integration.yml` use .NET 10, locked restore, audit, Release tests, portable validation, warnings-as-errors, clean-diff checks, and exact lowercase 40-character `hexagon.lock.json` pinning. Full local verification and source-bound evidence publication remain separate. | Lock files and portable/full scripts are present; intended protected contexts are `hexagon / neutral`, `hl2rp / integration`, and `sbox / release-evidence`. Before this final documentation-only commit, the lock had already advanced from the baseline to the exact pushed Hexagon remediation/evidence head; coordinated publication advances it again to this commit's pushed SHA. | Local implementation and exact-current local evidence are complete. Draft PRs, hosted runs, branch-protection enforcement, and pending release-evidence status publication are the remaining operational publication steps at this snapshot. |
